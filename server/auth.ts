...
import { validaCPF } from "../shared/utils";
    ...
    try {
      ...
      if (req.body.cpf) {
        ...
        if (!validaCPF(req.body.cpf)) {
          return res.status(400).send("CPF inválido. Por favor, forneça um CPF válido.");
        }
        
        // Verificar se o CPF já está em uso por outro usuário
        try {
          const existingCpfQuery = await storage.db.query(`
            SELECT * FROM users WHERE cpf = $1
          `, [req.body.cpf]);
          
          if (existingCpfQuery.rows.length > 0) {
            return res.status(400).send("Este CPF já está cadastrado em outra conta.");
          }
        } catch (err) {
          console.error("Erro ao verificar CPF:", err);
        }
      } else {
        return res.status(400).send("CPF é obrigatório para o cadastro.");
      }
