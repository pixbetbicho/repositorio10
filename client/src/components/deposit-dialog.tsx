...
export function DepositDialog({ onSuccess, open: controlledOpen, onOpenChange }: DepositDialogProps) {
  ...
  // Definir Ezzebank como gateway padrão quando os gateways forem carregados
  useEffect(() => {
    if (gateways.length > 0 && !form.getValues().gatewayId) {
      // Procurar gateway do tipo ezzebank
      const ezzebank = gateways.find(g => g.type === "ezzebank");
      if (ezzebank) {
        form.setValue("gatewayId", ezzebank.id.toString());
      } else {
        // Se não encontrar Ezzebank, usar o primeiro gateway disponível
  ...
  const depositMutation = useMutation({
    mutationFn: async (data: { amount: number, gatewayId: number }) => {
      ...
      if (gateway?.type === "pushinpay") {
        ...
        return await res.json();
      } else if (gateway?.type === "ezzebank") {
        // Ezzebank (PIX)
        const res = await apiRequest("POST", "/api/payments/ezzebank", {
          amount: data.amount
        });
        return await res.json();
      } else {
